...
def get_api_credentials():
    """Dynamically fetch API keys from secrets or session state override."""
    # Priority 1: Session State Override (Manual entry in Sidebar)
...
if 'watchlist' not in st.session_state:
    st.session_state.watchlist = ["EURUSD=X", "BTC-USD", "ETH-USD", "GC=F", "GBPUSD=X", "USDJPY=X", "^GSPC"]

# --- Market Utilities ---
def get_market_status():
    """Checks if major markets are open based on current UTC time."""
    now = datetime.now(pytz.utc)
    
    # Simple market hour check (Mon-Fri)
    is_weekend = now.weekday() >= 5
    
    def is_open(start_h, start_m, end_h, end_m, tz_name):
        if is_weekend: return False
        tz = pytz.timezone(tz_name)
        market_now = now.astimezone(tz)
        start = market_now.replace(hour=start_h, minute=start_m, second=0, microsecond=0)
        end = market_now.replace(hour=end_h, minute=end_m, second=0, microsecond=0)
        return start <= market_now <= end

    status = {
        "NYC": "OPEN" if is_open(9, 30, 16, 0, 'America/New_York') else "CLOSED",
        "LON": "OPEN" if is_open(8, 0, 16, 30, 'Europe/London') else "CLOSED",
        "TOK": "OPEN" if is_open(9, 0, 15, 0, 'Asia/Tokyo') else "CLOSED"
    }
    return status

# --- Functions ---

@st.cache_data(ttl=300, show_spinner="Skenuji trhy...")
def batch_fetch_watchlist(tickers):
    """Fetches all watchlist data in a single batch to avoid rate limits."""
    try:
...
def fetch_data(ticker_symbol, period, interval="1d"):
    """Fetches historical market data with interval support and retry logic for 429 errors."""
...
def fetch_fundamentals(ticker_symbol):
    """Fetches fundamental data from yfinance with retry logic."""
    ticker = yf.Ticker(ticker_symbol)
    max_retries = 2
    for attempt in range(max_retries):
        try:
...
def fetch_news(ticker_symbol):
    """Fetches latest news from yfinance with robust parsing and retry logic."""
    ticker = yf.Ticker(ticker_symbol)
    max_retries = 2
    for attempt in range(max_retries):
        try:
...
@st.cache_data(show_spinner=False, ttl=3600) # Cache for 1 hour to save quota
def generate_analysis(ticker, price_data_json, fundamentals_json, news_json=None):
    """Generates the AI analysis using the selected provider. Inputs are JSON strings for caching."""
    import io
    # Convert back from JSON for processing
    df = pd.read_json(io.StringIO(price_data_json))
    fundamentals = json.loads(fundamentals_json)
    news = json.loads(news_json) if news_json else None
        ...
        if provider == "OpenAI":
            ...
            for model_name in models_to_try:
                try:
                    ...
                    # Try to find the first '{' and last '}' if parsing fails initially
                    try:
...
with st.sidebar:
    ...
    with st.expander("⚙️ Engine & Panel Settings", expanded=True):
        ...
        # Always allow manual override if requested or if secrets fail
        with st.expander("🔑 API Key Override", expanded=not api_key):
            # Use session state to persist manual entry
            st.text_input("Vložte náhradní API Key:", type="password", key="manual_api_key", help="Pokud klíč v secrets nefunguje, vložte sem jiný.")
            st.radio("Provider:", ["Gemini", "OpenAI"], horizontal=True, key="manual_api_provider")
            
            if st.session_state.manual_api_key:
                api_key = st.session_state.manual_api_key
                ai_provider = st.session_state.manual_api_provider
                st.info(f"Používám ručně zadaný klíč ({ai_provider})")

        st.divider()
        st.markdown("##### 🛠️ System Tools")
        if st.button("Vymazat Cache Analýz", use_container_width=True, help="Vynutí stažení nových dat od AI."):
            st.cache_data.clear()
...
# 1.5. Watchlist Scanner (New Section)
# --- 1.5. Watchlist Scanner ---
st.markdown("<h3 style='font-size: 1.2rem; margin-top: 10px; margin-bottom: 15px;'>🛡️ Institutional Watchlist Scanner</h3>", unsafe_allow_html=True)

# Batch fetch all watchlist data once
watchlist_tickers = st.session_state.watchlist
all_w_data = batch_fetch_watchlist(watchlist_tickers)

scanner_cols = st.columns(len(watchlist_tickers))
for i, w_ticker in enumerate(watchlist_tickers):
    with scanner_cols[i]:
        # Extract individual ticker data from batch
        try:
            if isinstance(all_w_data.columns, pd.MultiIndex):
                w_df = all_w_data[w_ticker].dropna()
            else:
                w_df = all_w_data.dropna() # Fallback for single ticker
                
            if not w_df.empty and len(w_df) > 20:
                w_df = calculate_indicators(w_df)
                w_last = w_df.iloc[-1]
                w_prev = w_df.iloc[-2]
                
                w_price = float(w_last['Close'])
                w_change = ((w_price - w_prev['Close']) / w_prev['Close']) * 100
                w_rsi = w_last['RSI']
                
                # Enhanced Signal Logic
                is_bullish = w_price > w_last['SMA_50']
                is_uptrend = w_price > w_last['SMA_200']
                
                if is_bullish and is_uptrend and w_rsi < 45: signal = "STRONG BUY"
                elif is_bullish and w_rsi < 60: signal = "BUY"
                elif not is_bullish and not is_uptrend and w_rsi > 55: signal = "STRONG SELL"
                elif not is_bullish and w_rsi > 40: signal = "SELL"
                else: signal = "NEUTRAL"
                
                trend_color = "#10B981" if "BUY" in signal else "#EF4444" if "SELL" in signal else "#94A3B8"
                rsi_color = "#10B981" if w_rsi < 35 else "#EF4444" if w_rsi > 65 else "#94A3B8"
                
                # Fix the formatting error by pre-formatting
                if w_price >= 1: p_fmt = f"{w_price:,.2f}"
                elif w_price >= 0.01: p_fmt = f"{w_price:,.4f}"
                else: p_fmt = f"{w_price:,.6f}"

                with st.container(border=True):
                    st.markdown(f"""
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="font-size: 0.8rem; font-weight: 700; color: #F8FAFC;">{w_ticker.split('=')[0]}</div>
                            <div style="font-size: 0.6rem; font-weight: 600; color: {trend_color}; background: {trend_color}15; padding: 2px 6px; border-radius: 4px;">{signal}</div>
                        </div>
                        <div style="font-size: 1.1rem; font-weight: 800; color: #F8FAFC; margin-top: 5px;">${p_fmt}</div>
                        <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                            <span style="font-size: 0.65rem; color: #94A3B8;">RSI: <b style="color:{rsi_color}">{w_rsi:.0f}</b></span>
                            <span style="font-size: 0.65rem; color: {trend_color}; font-weight: 600;">{'+' if w_change > 0 else ''}{w_change:.2f}%</span>
                        </div>
                    """, unsafe_allow_html=True)
                    if st.button("Audit", key=f"audit_{w_ticker}", use_container_width=True):
                        st.session_state.active_ticker = w_ticker
                        st.rerun()
            else:
                st.markdown(f"<div style='font-size:0.7rem; color:#475569; padding:10px;'>{w_ticker}<br>No Data</div>", unsafe_allow_html=True)
        except Exception:
            st.markdown(f"<div style='font-size:0.7rem; color:#475569; padding:10px;'>{w_ticker}<br>Error</div>", unsafe_allow_html=True)

# Update global ticker from scanner if selected
if 'active_ticker' in st.session_state:
    ticker = st.session_state.active_ticker
    del st.session_state.active_ticker # Clear for next run
...
if ticker:
    ...
    if df_raw.empty:
        ...
        if generate_btn:
            # Final check of credentials (including potential manual overrides from session state)
            if not api_key:
                st.error("Zadejte prosím API klíč v Settings pro spuštění AI analýzy.")
            else:
                with st.spinner("Sběr dat a generování posudku..."):
                    try:
                        ...
                        # Convert data to JSON for the cached function (DataFrames aren't hashable)
                        price_json = df_processed.tail(100).to_json() # Only send last 100 candles to save context
                        fund_json = json.dumps(fundamentals)
                        news_json_str = json.dumps(news_context)
                        
                        ai_data = generate_analysis(ticker, price_json, fund_json, news_json=news_json_str)
